...
function ProductCategory({categories}) {
    ...
    return (
        <div className="rounded-2xl p-4.5 bg-lightGray3 font-Yekan-Medium ">
            <div onClick={() => setCloseCategory(prevState => !prevState)}
                 className={`flex justify-between  ${closeCategory && `border-b pb-4 `}  `}>
                <p className="font-Yekan-Medium text-sm">دسته بندی </p>
                <ChevronDown className={`${closeCategory && `-rotate-180`}  transition-all w-4 h-4`}/>
            </div>
            <div
                className={`${!closeCategory ? `opacity-0 h-0 p-0 invisible` : `h-auto py-6 opacity-100 visible`} flex flex-col gap-4 transition-all`}>
                {
